...
def run_from_dataframe(
    transactions: pd.DataFrame, limit_rows: int = 20, show_plots: bool | None = None
) -> pd.DataFrame:
...
def run_from_csv(
    csv_path: str, limit_rows: int = 20, show_plots: bool | None = None
) -> pd.DataFrame:
    transactions = pd.read_csv(csv_path)
    return run_from_dataframe(
        transactions=transactions, limit_rows=limit_rows, show_plots=show_plots
    )


def run(csv_path: str, limit_rows: int = 20, show_plots: bool | None = None) -> pd.DataFrame:
    # Backward-compatible alias for existing notebook cells.
    return run_from_csv(csv_path=csv_path, limit_rows=limit_rows, show_plots=show_plots)
